    ...
    open var imageURL: String? {
        didSet {
            if let _imageURL = imageURL {
                ...
                self.imageView?.sd_setImage(with: URL(string: _imageURL), placeholderImage: UIImage(named: imagePlaceHolder))

                if self.originFrame == CGRect.zero {
                    let imageViewH = (self.imageView?.image?.size.height)! / (self.imageView?.image?.size.width)! * screenWidth
                    self.imageView?.bounds = CGRect(x: 0, y: 0, width: screenWidth, height: imageViewH)
                    self.imageView?.center = (scrollView?.center)!
                } else {
                    self.imageView?.frame = self.originFrame
...
//            let offsetY = center.y - self.touchY
    ...
    @objc func singleTapClick(_ tap: UITapGestureRecognizer) {
        ...
        UIView.animate(withDuration: animDuration, delay: 0, options: UIViewAnimationOptions.allowUserInteraction, animations: {
//                self.imageView?.frame = self.originFrame
...
//                UIApplication.sharedApplication().keyWindow?.windowLevel = UIWindowLevelNormal
    ...
    open func show() {
        ...
        UIView.animate(withDuration: self.animDuration, delay: 0, options: UIViewAnimationOptions.allowUserInteraction, animations: {
                self.imageView?.bounds = CGRect(x: 0, y: 0, width: self.screenWidth, height: (self.imageView?.image?.size.height)! / (self.imageView?.image!.size.width)! * self.screenWidth)
